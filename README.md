...
- **新增模型**：
  - Qwen2.5-VL 72B: 72B 稠密多模态视觉模型，h=8192, layers=80, kvHeads=8, 128K 上下文
...
- 参数来源：魔搭社区官方 config.json
- **Bug 修复**：
  - 修复切换模型时不触发计算和零并发告警的问题
  - 修复 Qwen3.5 397B-A17B 上下文长度选项为空的问题（256K 动态添加）
- **清理重复**：删除旧的 Qwen3.5 397B 条目（与新模型参数相同）

### v2.1（2026-05-24）

- **Vercel 部署支持**：
  - 新增 `vercel.json` 配置文件
  - 新增 `dist/` 部署目录，包含所有静态资源
  - 支持 Vercel 自动构建和部署
